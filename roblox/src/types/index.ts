...
export namespace NetValidation {
	...
	export type InvokerFromValidatorsArray<T extends ReadonlyArray<check<unknown>>> = (
	 ...
	 */
	export const str: check<string> = (value): value is string => typeIs(value, "string") && utf8_validator(value);
	...
	export function none(value: unknown): value is undefined {
	...
	export const EnumItem =
		<E extends Enum>(rbxEnum: E): check<ReturnType<E["GetEnumItems"]>[number]> =>
		(value): value is ReturnType<E["GetEnumItems"]>[number] => {
			return typeIs(value, "EnumItem") && value.EnumType === rbxEnum;
		};

	function valuesOf<T extends object>(object: T): ReadonlyArray<NonNullable<T[keyof T]>> {
	...
	export const TSEnum =
		<K>(enumLike: { [P in keyof K]: K[P] }): check<K[keyof K]> =>
		(value: unknown): value is K[keyof K] => {
			const enumValues = valuesOf(enumLike) as unknown as Array<string | number>;
	...
	const arrayLike: check<Array<defined>> = (value): value is Array<defined> =>
		typeIs(value, "table") && typeIs(next(value)[0], "number");

	const mapLike: check<Map<defined, unknown>> = (value): value is Map<defined, unknown> =>
		typeIs(value, "table") && typeIs(next(value)[0], "string");

	const tbl: check<object> = (value): value is object => typeIs(value, "table");

	const keys = <K>(keyType: check<K>) => {
		return (value: unknown): value is Map<K, unknown> => {
			if (!tbl(value)) {
				return false;
			}
	...
	const values = <V>(valueType: check<V>) => {
		return (value: unknown): value is Map<defined, V> => {
			if (!tbl(value)) {
				return false;
			}

			for (const [, val] of pairs(value)) {
				if (!valueType(val)) {
					return false;
				}
			}
	...
	export const array =
		...
		(value: unknown): value is Array<T> => {
			if (!keys(number)(value)) return false;
			return arrayLike(value) && value.every((v) => valueType(v));
		};

	export const map =
		<K, V>(keyType: check<K>, valueType: check<V>) =>
		(value: unknown): value is Map<K, V> => {
			if (!keys(keyType)(value)) return false;
			if (!values(valueType)(value)) return false;
...
if (NetValidation.map(NetValidation.Player, NetValidation.number)(test)) {
}
